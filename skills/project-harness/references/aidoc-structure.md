...
- `lessons/README.md` + `lessons/TEMPLATE.md`: staging area for pitfalls and recurring patterns. Each lesson carries a header `lesson-meta` marker (`status`: pending/deferred/promoted/dropped, `count`, `post`, `target`) — the sole basis for `check_sync.py`'s mechanical lesson scan; the human-readable sections carry context, pitfall/pattern, occurrence count, status, promotion target, post-promotion recurrence, record date. Promotion discipline (second occurrence or user confirmation → rule written into the constraint body's home document in the same change) and the post-promotion recurrence loop are defined in [change-docs.md](change-docs.md); `lessons/README.md` states the capture rules and carries the lesson index pointer.
