...
3. **决策要留痕、约束要回写**：改变行为、架构、共享契约、流程、测试策略或持久数据的变更，必须按 `aiDoc/notes/README.md` 的规则新增或更新决策记录；若决策涉及架构/契约/流程约束，必须在同一变更内同步更新约束正文所在文档（`aiDoc/modules/architecture-rules.md`、`aiDoc/contracts/boundary.md` 或本文件），禁止只记决策不改规则。
...
8. **改动收尾必过漂移自检**：完成任何代码或文档改动前，必须主动做漂移自检并处理结果——本仓库运行 `python3 skills/project-harness/scripts/check_sync.py .`，无脚本时按 `aiDoc/README.md` 路由表核对受影响文档。发现漂移必须在同一变更内回写修复，不得留待下次；自检结果是 Definition of Done 的一部分。
9. **踩坑与反复模式必须入库**：工作中踩坑、review 反复发现同类问题、或发现可复用模式时，必须在同一变更内按 `aiDoc/memory/lessons/README.md` 记一条 lesson；已有同类 lesson 时就地累加出现次数，第二次出现时立即按晋升纪律写进约束正文。禁止只凭口头记忆不带留痕。`check_sync.py` 对 lessons 做机械扫描：`pending` 且次数 ≥2 未晋升也未显式暂缓（deferred）、或 `promoted` 缺晋升去向，会使漂移自检失败。
...
- [ ] 文档中的路径、符号引用与真实代码一致
- [ ] 改动收尾已做漂移自检（check_sync.py 或路由表核对），发现的漂移已在同一变更内处理
