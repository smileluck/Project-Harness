...
9. **踩坑与反复模式必须入库**：工作中踩坑、review 反复发现同类问题、或发现可复用模式时，必须在同一变更内按 `aiDoc/memory/lessons/README.md` 记一条 lesson；已有同类 lesson 时就地累加出现次数，第二次出现时立即按晋升纪律写进约束正文。禁止只凭口头记忆不带留痕。
