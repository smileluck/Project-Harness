...
3. 同步 `references/` 中描述该能力的文档（如探测矩阵表）。
4. 若改了产物结构（新增/删除生成文件），同步 `templates/zh|en`、`check_sync.py` 检查逻辑、`references/aidoc-structure.md` 三处。

## 新增工作流（references 新篇）

1. 在 `references/` 新建英文文档，相对路径交叉引用。
2. 在 `SKILL.md` 路由表登记（一行说明 + 链接）。
3. 若工作流产生新的 aiDoc 区域，先改 `references/aidoc-structure.md` 的目录契约，再改模板。

## 新增/修改模板区域

1. `templates/zh/` 与 `templates/en/` 同改，保持文件清单与结构镜像。
