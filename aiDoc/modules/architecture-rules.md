...
- **一个事实一个维护家**：aiDoc 目录契约只在 `skills/project-harness/references/aidoc-structure.md`；探测/扫描逻辑只在 `skills/project-harness/scripts/scan_repo.py`；他处只引用。
...
## Skill 层（`SKILL.md` + `skills/project-harness/references/`）
...
## 模板层（`skills/project-harness/templates/zh/` + `skills/project-harness/templates/en/`）
